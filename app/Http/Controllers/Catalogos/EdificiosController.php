...
class EdificiosController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ($edificio != "") {
            $data['edificios'] = Edificio::with('administracion','colonia','alcaldia','entidad','codigo_postal')->where('cdescripcion_edificio','like','%'.$edificio.'%')->where('iestatus','=',1)->get();
        } else {
    ...
    {
        if ($request->descripcion_edificio!="") {
        //Revisar que no exista un Edificio con la misma Descripción
            $ya_hay_edificio                            = Edificio::where('cdescripcion_edificio','=',$request->descripcion_edificio)
